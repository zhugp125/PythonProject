...
if __name__ == '__main__':
    ...
    # 获取视频的名称
    filename = get(text, r'<h6 class="font-20 title">(.*)</h6>')
    #print('url: {}\nfile: {}'.format(url, filename))

    req = download(url)
    if not req.text.startswith('#EXTM3U'):
        raise BaseException('m3u8 error url')

    url_pre = url.rsplit('/', 1)[0]
    #print('url', url_pre)
    # 加密类型
    encry = get(req.text, r'#EXT-X-KEY:METHOD=(.*),URI=".*"')
    # key
    key = get(req.text, r'#EXT-X-KEY:METHOD=.*,URI="(.*)"')
